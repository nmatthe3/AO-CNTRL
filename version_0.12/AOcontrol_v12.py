...
class AOunit(QObject):
    ...
    def __init__(self):
        ...
        self.asi_lib_path = "../ASI_Windows_SDK_V1.28/ASI SDK/lib/x64/ASICamera2.dll"
    ...
    def move_north(self):
        ...
        self.check_limit()
        self.north_south_clicks += self.AO_steps_per_cmd
        self.n_clicks += 1
        self.print_position()
        self.wait()
        return
    ...
    def move_east(self):
        ...
        self.check_limit()
        self.n_clicks += 1
        self.east_west_clicks += self.AO_steps_per_cmd
        self.print_position()
        self.wait()
        return
...
class AO_interface(QWidget):
    ...
    def init_UI(self):
        ...
        self.update_N_Steps_B.clicked.connect(self.updateNSteps)
    ...
    def updateNSteps(self):
        """Updates the number of AO steps for each call"""
        CMD_value = str(self.N_Steps_L.text())
        print("Updating AO steps to %s" % CMD_value)
        num = CMD_value.zfill(5)
        self.AO.AO_steps_per_cmd = int(CMD_value)
        self.AO.cmd_north = bytes("GN"+num,"utf-8")
        self.AO.cmd_south = bytes("GS"+num,"utf-8")
        self.AO.cmd_east  = bytes("GT"+num,"utf-8")
        self.AO.cmd_west  = bytes("GW"+num,"utf-8")
        return
    ...
    def threaded_control_algorithm(self):
        #Create new thread when running CA so GUI does not freeze
        self.thread = QThread()
        self.thread.start()
